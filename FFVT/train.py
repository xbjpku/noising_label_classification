...
def setup(args):
    ...
    smoothing_value=args.smoothing_value, dataset=args.dataset, peer=args.peer, noise_rate=args.noise_rate, loss_type=args.loss_type)
...
def train(args, model):
    ...
    while True:
        ...
        for step, batch in enumerate(epoch_iterator):
            batch = tuple(t.to(args.device) for t in batch)
            x, y = batch
            loss, logits = model(x, y, global_step)
...
def main():
    ...
    seed = args.seed * dist.get_world_size() + args.local_rank
