...
class CUB():
    def __init__(self, root, is_train=True, data_len=None, transform=None, label_file = "training_labels_noise_20.txt"):
        self.root = root 
        ...
        if self.is_train:
            ...
            self.train_label = [label_list[int(os.path.splitext(i)[0])] for i in img_name_list][:data_len]
            self.train_imgname = [x for x in img_name_list[:data_len]]
        if not self.is_train:
            ...
            self.test_img = [os.path.join(img_path, test_file) for test_file in
                             img_name_list[:data_len]]
            self.test_label = [label_list[int(os.path.splitext(i)[0])] for i in img_name_list][:data_len]
            self.test_imgname = [x for x in img_name_list[:data_len]]
