    ...
    if args.dataset == "cifar10":
        ...
        test_transform=transforms.Compose([
            ...
            #transforms.Resize((args.img_size, args.img_size)),
                                    ...
                                    #transforms.Normalize([0.8416, 0.867, 0.8233], [0.2852, 0.246, 0.3262])])
                                    transforms.Normalize([0.485, 0.456, 0.406], [0.229, 0.224, 0.225])])
        trainset = eval(args.dataset)(root=args.data_root, is_train=True, transform=train_transform, label_file = f"training_labels_noise_{str(int(args.noise_rate * 100)).format('%02d')}.txt")
        testset = eval(args.dataset)(root=args.data_root, is_train=False, transform = test_transform)
