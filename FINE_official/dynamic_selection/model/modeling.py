...
class VisionTransformer(nn.Module):
    def __init__(self, config, img_size=224, num_classes=21843, smoothing_value=0, zero_head=False, vis=False, dataset='cotton', peer=True, noise_rate = 0):
    ...
    def forward(self, x, labels=None, step=None):
        ...
        if labels is not None:
            if self.smoothing_value == 0:
                # if self.peer:
                #     loss_fct = PeerLoss(alpha=self.noise_rate / 2, num_classes=self.num_classes, step=step)
                # else:
